...
public class GraphController {
    ...
    @FXML
    private CheckBox time_ch;
    @FXML
    private CheckBox mid_ch;
    ...
    private boolean timeOrDistance=false; // time is true distance is false
    private boolean midPoint=false; //false means no mid point 


    // used for A*
    private Stop startLocation;
    private Stop goalLocation;
    private Stop midLocation;
    ...
    public void handleMid(ActionEvent event){
        System.out.println("mid with event" + event.getEventType());

        if(mid_ch.isSelected()){
            midPoint=true;
        }else{
            midPoint=false;
        }
    ...
    public void handleStartAction(ActionEvent event) {
        ...
        if(startLocation!=null && goalLocation!=null){
            pathEdges = AStar.findShortestPathEdges(graph, startLocation, goalLocation,timeOrDistance);
    ...
    public void handleShowConnectedComponents(ActionEvent event) {
        ...
        tripText.setText("Number of connected components: "+graph.comp);
        drawGraph(graph);
    }

    public void handleTime(ActionEvent event){
        System.out.println("In time or distance " +time_ch.isSelected());
        if(time_ch.isSelected()){
            timeOrDistance=true;
        }else{
            timeOrDistance=false;
        }
    }
    ...
    public void handleAddWalking(ActionEvent event) {
        ...
        tripText.setText("Walking edges added: "+graph.count);
        drawGraph(graph);
    }
    ...
    public void handleMouseClick(MouseEvent event) {
        ...
        if(midPoint){
            startLocation=midLocation;
            midLocation=goalLocation;
            goalLocation=closestStop;
            System.out.println(midLocation.getName());

            if (startLocation != null && closestStop != startLocation) {
                // INFO: This is where your find path code is called during clicking
                pathEdges = AStar.findShortestPathEdges(graph, startLocation, midLocation,timeOrDistance);
                pathEdges.addAll( AStar.findShortestPathEdges(graph, midLocation, goalLocation,timeOrDistance));
    ...
    public void drawPathEdges(ArrayList <Edge> pathEdges, GraphicsContext gc) {
        ...
        // TODO: add total time to the path
            ArrayList<Edge> pathEdgesReveresed =pathEdges;
            Collections.reverse(pathEdgesReveresed);
            double total=time;
            double decimal = total/60;
                double sec = decimal%1;
             
                double minit = decimal-sec;
                sec*=60;

                String totalTime= String.format("%.0f : %.0f", minit,sec);
           
            for(Edge edge:pathEdgesReveresed){
                double dec = time/60;
                double seconds = dec%1;
             
                double mins = dec-seconds;
                seconds*=60;
                String timeMan= String.format("%.0f : %.0f", mins,seconds);
                s+=edge.getFromStop().getId()+ " : " +edge.getToStop().getId()+ "       "+ edge.getTripId()+"     Cost: " +edge.getTime() +"    to Goal: "+timeMan+"     Comp: " +edge.getFromStop().getSubGraphId()+ "\n";
                time-=edge.getTime();
            }
     
            s+= path;
            if(midPoint){
                tripText.setText("Start: " + startLocation.getName() + "\n" + "Total time: "+ totalTime+"\n"+"Mid Point: "+ midLocation.getName()+"\n"+s);
            }else{tripText.setText("Start: " + startLocation.getName() + "\n" + "Total time: "+ totalTime+"\n"+s);}
    ...
    public void drawGraph(Graph graph) {
        ...
        if (pathEdges != null) {
            ...
            System.out.println("DOING DRAW PATH EDGES");
